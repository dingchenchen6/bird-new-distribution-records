...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath("code/run_bird_new_record_hazard_model_empirical_recent_test.R", mustWork = TRUE)
}
...
output_root <- file.path(task_root, "empirical_hazard_recent_test")
...
paths <- list(
  new_record_csv = file.path(output_root, "derived_inputs", "ndr_raw_empirical_recent_test.csv"),
  sdm_province_csv = file.path(output_root, "derived_inputs", "sdm_province_empirical_recent_test.csv"),
  province_year_climate_csv = file.path(output_root, "derived_inputs", "province_year_climate_empirical_recent_test.csv"),
  species_year_native_climate_csv = file.path(output_root, "derived_inputs", "species_year_native_climate_empirical_recent_test.csv"),
  effort_csv = file.path(output_root, "derived_inputs", "effort_py_empirical_recent_test.csv"),
  species_trait_csv = file.path(output_root, "derived_inputs", "species_trait_empirical_recent_test.csv"),
  config_csv = file.path(output_root, "hazard_model_config_empirical_recent_test.csv")
)
...
writeLines(
  c(
    ...
    paste0("year_min,", year_min),
    paste0("year_max,", year_max),
    "model_engine,lme4"
  ),
...
coverage_tbl <- tibble(
  metric = c(
    "new_records_original_window",
    "new_records_supported_window",
    "species_original_window",
    "species_supported_window",
    "sdm_species_window",
    "province_year_climate_rows_window",
    "effort_rows_window"
  ),
  value = c(
    nrow(ndr_raw),
...
write_csv(coverage_tbl, file.path(output_root, "diagnostics", "empirical_recent_test_input_coverage.csv"))

message("Empirical recent hazard test completed.")
