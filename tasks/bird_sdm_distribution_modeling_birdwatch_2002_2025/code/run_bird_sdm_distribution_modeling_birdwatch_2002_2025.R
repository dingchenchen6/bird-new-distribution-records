...
make_repo_relative_path <- function(path) {
  ...
  vapply(values, function(one_path) {
    ...
    if (startsWith(normalized_path, paste0(task_root_normalized, "/"))) {
      return(file.path("tasks", "bird_sdm_distribution_modeling_birdwatch_2002_2025", substring(normalized_path, nchar(task_root_normalized) + 2L)) %>% normalize_path_slashes())
    }
...
BIRDWATCH_YEAR_START <- 2002L
BIRDWATCH_YEAR_END <- 2025L
BIRDWATCH_MERGED_OCCURRENCE_PATH <- file.path(OCCURRENCE_INPUT_DIR, "china_birdwatch_observation_records_2002_2025.csv.gz")
...
load_occurrence_points_from_birdwatch_raw <- function(china_boundary, species_master = NULL, overrides_tbl = NULL) {
  ...
  merged_tbl <- species_tbl %>%
    ...
    mutate(
      ...
      occurrence_source = "china_birdwatch_dataset_2002_2025"
    ) %>%
  ...
  readr::write_csv(
    tibble(
      ...
      merged_rows_2002_2025 = nrow(merged_tbl),
      unique_species_2002_2025 = n_distinct(merged_tbl$shp_species),
...
run_pipeline <- function(prepare_only = FALSE, selected_species = NA_character_, species_limit = NA_integer_, force_download = FALSE, current_only = FALSE, workers = 1L) {
  ...
  safe_message("Loading birdwatch occurrence points ...")
